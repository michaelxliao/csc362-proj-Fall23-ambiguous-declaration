DELIMITER //
CREATE OR REPLACE PROCEDURE add_reservation(patronid INT, spaceid INT, start_time DATETIME, end_time DATETIME,
notes VARCHAR(256))
 BEGIN
 START TRANSACTION;
...
DELIMITER //
CREATE OR REPLACE PROCEDURE update_reservation(id INT, patronid INT, spaceid INT, start_time DATETIME, end_time DATETIME,
notes VARCHAR(256))
 BEGIN
 START TRANSACTION;
    UPDATE space_reservations
        SET space_reservations.space_id = spaceid
        SET space_reservations.patron_id = patronid
        SET space_reservations.start_reservation = start_time
        SET space_reservations.end_reservation = end_time
...
DELIMITER //
CREATE OR REPLACE PROCEDURE add_club_reserve(reserveid, clubid)
 BEGIN
 START TRANSACTION;
    INSERT INTO club_reservations(reservation_id INT, club_id INT)
    VALUES
    (
        reserve_id, club_id
    )
...
DELIMITER //
CREATE OR REPLACE PROCEDURE update_club_reserve(reserveid, clubid)
 BEGIN
 START TRANSACTION;
    UPDATE club_reservations
        SET club_reservations.reservation_id = reserveid
        SET club_reservations.club_id = clubid
...
DELIMITER //
CREATE OR REPLACE PROCEDURE del_club_reserve(reserveid, clubid)
 BEGIN
 START TRANSACTION;
