...
$sql_query = 'SELECT * FROM pretty_past_space_reservations'; // this by default; can be overridden by filtering

$result = $conn->query($sql_query);
if (isset($_GET["Filter"])) {
$where_clauses = ["`End Time` < CURRENT_TIMESTAMP()", 
                    "patron_id = ?"];
    ...
    $space_stmt = $conn->prepare($final_sql_query);
    $space_stmt->bind_param("i", $login_id);
    if(!$space_stmt->execute())
    {
        echo "SQL happened /neg";
        exit();
    }

    $result = $space_stmt->get_result();
    if (!$result) {
    ...
    $final_sql_query = $sql_query . ' WHERE `End Time` < CURRENT_TIMESTAMP() AND patron_id = ?;';
    $space_stmt = $conn->prepare($final_sql_query);
    $space_stmt->bind_param("i", $login_id);
    if(!$space_stmt->execute())
    {
...
<body>
    ...
    <form method=GET> <!-- NOTE FOR FILTERING: don't freakin forget your quotes -->
        ...
        <?php result_to_deletable_table_general($result, [-1], "Delete","Delete Reservations"); ?>
